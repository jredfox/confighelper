...
public class DimensionManager
{
    ...
    public static int[] unregisterProviderType(int id)
    {
        ...
        int x = 0;
        for (Map.Entry<Integer, Integer> ent : getDimensions().entrySet())
        {
            if (ent.getValue() == id)
            {
                ret[x++] = ent.getKey();
    ...
    {
        return getDimensions().containsKey(dim);
    }
    ...
    {
        return getDimensions().keySet().toArray(new Integer[getDimensions().keySet().size()]);
    }
    
    public static Integer[] getStaticProviderIDs()
    {
        return getProviders().keySet().toArray(new Integer[getProviders().keySet().size()]);
    }
    ...
    public static WorldProvider createProviderFor(int dim)
    {
        try
        {
            if (getDimensions().containsKey(dim))
            {
                WorldProvider provider = getProviders().get(getProviderType(dim)).newInstance();
        ...
        catch (Exception e)
        {
            FMLCommonHandler.instance().getFMLLogger().log(Level.SEVERE,String.format("An error occured trying to create an instance of WorldProvider %d (%s)",
                    dim, getProviders().get(getProviderType(dim)).getSimpleName()),e);
    ...
    */
    ...
    public static int getNextFreeDimId() {
        ...
        while (true)
        {
            ...
            if (getDimensions().containsKey(next))
            {
    ...
    public static void loadDimensionDataMap(NBTTagCompound compoundTag)
    {
        ...
        {
            dimensionMap.clear();
            for (Integer id : getDimensions().keySet())
            {
                if (id >= 0)
                {
                    dimensionMap.set(id);
	...
	public static Hashtable<Integer, Integer> getDimensions() {
		return dimensions;
	}

	public static void setDimensions(Hashtable<Integer, Integer> dimensions) {
		DimensionManager.dimensions = dimensions;
	}
}
